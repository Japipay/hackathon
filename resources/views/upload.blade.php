...
<body class="bg-gray-900 text-white">
        ...
        <div class="flex space-x-4">
            ...
            {{--
             <a href="{{ route('quiz.create') }}"
               class="px-3 py-2 rounded-md text-sm font-medium 
               {{ request()->routeIs('quiz.create') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:text-white' }}">
                Quiz AI
            </a> 
            --}}
    ...
    <div class="main-container w-full max-w-4xl">
        ...
        <div class="flex gap-4 mb-6">
            ...
            <button type="button" class="btn bg-green-600 text-white px-4 py-2 rounded w-full shadow" onclick="openFlashcardsModal()">
                <i class="fas fa-cards mr-2"></i> Generate Flashcards
            </button>
        </div>

        <!-- Flashcards Modal -->
        <div class="fixed inset-0 flex items-center justify-center hidden bg-gray-900 bg-opacity-75" id="flashcardsModal">
            <div class="card w-96">
                <h5 class="text-xl font-semibold text-white mb-4 text-center">Flashcard Options</h5>
                <form action="{{ route('generate.flashcards') }}" method="POST" enctype="multipart/form-data" id="flashcardsForm">
                    @csrf
                    <input type="hidden" name="uploaded_pdf" value="{{ session('uploaded_pdf') }}">
                    
                    <div class="mb-3">
                        <label class="block text-gray-300 mb-1">Number of Flashcards</label>
                        <input type="number" class="w-full p-2 border rounded bg-gray-700 text-white" name="flashcard_count" min="3" max="20" value="10" id="flashcardCount">
                    </div>
                    <div class="mb-3">
                        <label class="block text-gray-300 mb-1">Difficulty Level</label>
                        <select class="w-full p-2 border rounded bg-gray-700 text-white" name="difficulty" id="difficulty">
                    ...
                    <div class="flex gap-4">
                        <button type="button" class="btn bg-gray-600 text-white px-4 py-2 rounded w-full shadow" onclick="closeFlashcardsModal()">Cancel</button>
                        <button type="submit" class="btn bg-blue-600 text-white px-4 py-2 rounded w-full shadow">Generate Flashcards</button>
                    </div>
                </form>
            </div>
        </div>
